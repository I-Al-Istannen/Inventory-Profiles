...
public class BuySign implements ConfigurationSerializable {
	 ...
	 */
	public BuySign(Location location, String regionID, double price, boolean wallSign, BlockFace
		...
		Bukkit.getScheduler().runTaskLater(InventoryProfiles.getInstance(), this::update, 1);
	}
	...
	public BuySign(Map<String, Object> map) {
		this(((LocationSerializable) map.get("location")).toLocation(),
				(String) map.get("regionID"),
				((Number) map.get("price")).doubleValue(),
				map.containsKey("wallSign") && (boolean) map.get("wallSign"),
				map.containsKey("facingDirection")
						? BlockFace.valueOf((String) map.get("facingDirection"))
						: BlockFace.SOUTH);
	}
	...
	public void update() {
		...
		if ((location.getBlock().getType() != Material.SIGN_POST
			...
			if (isWallSign()) {
				// prevent the first, pointless checking caused by the RegionObject creating a new sign with the
				// second constructor.
	...
	public Optional<BlockFace> getBlockFace() {
		if (location.getBlock().getType() != Material.SIGN_POST && location.getBlock().getType() != Material
				.WALL_SIGN) {
			return Optional.empty();
		}
		return Optional.of(((org.bukkit.material.Sign) location.getBlock().getState().getData()).getFacing());
	}
