...
public class ConversationManager {
	 ...
	 */
	public boolean startConversation(Player player, ConversationType type, ConversationAbandonedListener
			abandonListener, String promptText) {
		if (player.isConversing()) {
			return false;
		}

		if (type == ConversationType.LOGIN) {
			Conversation conversation = factory.withTimeout(type.getTimeoutSeconds()).withFirstPrompt(new
					LoginConversation()).buildConversation(player);
			...
			Conversation conversation = factory.withTimeout(type.getTimeoutSeconds()).withFirstPrompt(new
					ConfirmationConversation(promptText)).buildConversation(player);
	 ...
	 */
	public boolean startConversation(Player player, ConversationType type,
	...
	public void cancelAllConversations() {
		activeConversations.values().forEach(Conversation::abandon);
	}
	...
	public enum ConversationType {
		...
		ConversationType(int timeoutSeconds) {
